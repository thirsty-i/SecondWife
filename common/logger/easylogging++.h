...
public:
    ...
    std::ostream& setColor(Level level, std::ostream& stream)
    {
        switch (level)
        {
        case Level::Trace:
        ...
        if (m_data->dispatchAction() == base::DispatchAction::NormalLog) {
            ...
            if (m_data->logMessage()->logger()->m_typedConfigurations->toStandardOutput(m_data->logMessage()->level())) {
                ...
                if (ELPP->hasFlag(LoggingFlag::ColoredTerminalOutput)) {
                    std::ostream& stream = m_data->logMessage()->logger()->logBuilder()->setColor(m_data->logMessage()->level(), ELPP_COUT);
                    stream << ELPP_COUT_LINE(logLine) << termcolor::reset;
                }
                else{
